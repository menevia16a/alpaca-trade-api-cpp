    ...
    */
    ...
    public:
        ...
        */
        ...
        /**
        * @brief Fetch latest trades for a stock from Alpaca
        *
        * @code{.cpp}
        ...
        * @param symbols
        * @return a std::pair where the first element is a Status indicating the
        * success or failure of the operation and the second element is an std::unordered_map
        * with key is an std::string (the stock symbol) and the value is an alpaca::Trade object.
        */
        std::pair<Status, std::unordered_map<std::string, Trade>> getLatestTrades(const std::vector<std::string>& symbols, const std::string& userAgent = "") const;

        /**
        * @brief Parses a timestamp from Alpaca into a Unix timestamp
        *
        * @code{.cpp}
        *   std::tm tm = {};
        ...
        *
        * @param timestamp
        * @return returns a uint64_t representing a Unix timestamp value.
        */
