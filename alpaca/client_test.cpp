...
TEST_F(ClientTest, testGetAccountConfigurations) {
    ...
    auto update_account_configurations_resp = client.updateAccountConfigurations(false, account_configurations.dtbp_check, account_configurations.trade_confirm_email, account_configurations.suspend_trade);
...
TEST_F(ClientTest, testOrders) {
    ...
    for (int i = 0; i < number_of_orders; i++) {
        ...
        auto submit_order_response = client.submitOrder("NFLX", 10, alpaca::OrderSide::Buy, alpaca::OrderType::Market, alpaca::OrderTimeInForce::Day);
...
TEST_F(ClientTest, testPositions) {
    ...
    auto symbol = "NFLX";
    auto number_of_orders = 3;
    auto close_positions_response = client.closePositions(); // liquidate all existing positions
    ...
    for (int i = 0; i < number_of_orders; i++) {
        // submit a market order to buy 10 shares of NFLX
        auto submit_order_response = client.submitOrder(symbol, 10, alpaca::OrderSide::Buy, alpaca::OrderType::Market, alpaca::OrderTimeInForce::Day);
...
TEST_F(ClientTest, testWatchlists) {
    ...
    auto create_watchlist_response = client.createWatchlist("My Test Watchlist - " + alpaca::randomString(24), std::vector<std::string>{"GOOG", "WORK", "MSFT", "AAPL", "FB", "NFLX", "TSLA", "TWTR", "NOW", "DIS"});
